 ...
 * @LastEditTime: 2021-09-30 11:54:12
...
const whiteList = ['/login', '/auth-redirect', '/Dialog', '/testDialog', '/inspectionTour', 'organization', 'DamMonitoring', 'subskMap'] // 白名单
...
router.beforeEach(async(to, from, next) => {
  ...
  console.log(subdsysID, 'subdsysID====888999郭慧娜')
  if (!hasToken) {
    console.log(hasToken, '!hasToken')
    ...
    if (to.path === '/login') {
      ...
      if (hasMenus) {
        ...
        try {
          ...
          // if (subdsysID != null) {
          const menus = await store.dispatch('user/getMenus', sysmenus)
