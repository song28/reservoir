...
export default {
  ...
  watch: {
    DistrictLng: {
      handler(newV, oldV) {
        let zoom = 6;
        if (this.changeMapLevel == 1) {
          zoom = 6;
        } else if (this.changeMapLevel == 2) {
          zoom = 10;
        } else if (this.changeMapLevel == 3) {
          zoom = 13;
        }
        if (this.changeMapLevel != 4) {
  ...
  methods: {
    ...
    async showVideoWebApp(item) {
      ...
      this.showOpenVideoWebAppData.token = await this.getToken(
        item.data.userId
      );
      window.open(
        openVideoWin +
    ...
    loadMap() {
      ...
      var satelliteLabelLayer = new TileLayer({
        ...
        source: new XYZSource({
          url: "http://t3.tianditu.com/DataServer?T=img_w&x={x}&y={y}&l={z}&tk=2e089c2bb3c79e23cb1038ca8112cb37",
      ...
      register(proj4);
      const imageExtent = [
        65.9796331669, 10.7451916711, 140.1812559169, 58.4673388687,
      ];
    ...
    loadSwitcherMap(val) {
      ...
      for (var i = 0; i < self.map.getLayers().getArray().length; i++) {
        for (let z = 0; z < layerNames.length; z++) {
          if (
            layerNames.indexOf(
              self.map.getLayers().getArray()[i].get("name")
            ) != -1
          ) {
            self.map
              .getLayers()
              .getArray()
              [i].setVisible(
                self.map.getLayers().getArray()[i].get("name") == val.layer_id
              );
          }
        }
      }
    ...
    mapToolClick(data) {
      ...
      switch (data.type) {
        ...
        case "mapReset":
          ...
          this.map
            .getView()
            .setCenter([this.DistrictLng[0], this.DistrictLng[1]]);
          break;
        case "zoomIn":
          view.setZoom(zoom + 1);
          break;
    ...
    async addPop(data, type) {
      ...
      if (type === "user") {
        ...
        const address =
          data.currentLoc == null ? "" : "(" + data.currentLoc + ")";
    ...
    addClickDialog() {
      ...
      that.map.on("singleclick", function (evt) {
        ...
        const features = that.map.forEachFeatureAtPixel(
          pixel,
