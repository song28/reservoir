<template>
  <div class="containerMap">
   ...
   ></iframe>-->
    ...
    <!-- 左侧tool -->
    <div class="map-btn-box" :style="{left:leftDis}">
      <map-control
        :map-type="mapType"
...
export default {
  ...
  watch: {
    mapCenter: {
      handler(newV, oldV) {
        ...
        view.setZoom(18)
        this.map.getView().setCenter([newV[0], newV[1]])
        console.log(newV, '地图中心点')
      },
      deep: true,
      immediate: true
    },
    ...
    markert: {
      handler(newV, oldV) {
        this.$nextTick(() => {
          if (newV.length) {
            // console.log(newV, 'mardddddddddd')
  ...
  methods: {
    ...
    async showVideoWebApp(item) {
      ...
      console.log(this.showOpenVideoWebAppData, 'token===')
      window.open(
        openVideoWin +
    ...
    loadMap() {
     ...
     '+units=m +no_defs'
      ...
      console.log([this.mapCenter[0], this.mapCenter[1]], '图土对对对对对对多')
      self.map = new Map({
        target: 'map',
        layers: layers,
        view: new View({
          center: [self.mapCenter[0], self.mapCenter[1]], // 地图中心点坐标
    ...
    mapToolClick(data) {
      ...
      switch (data.type) {
        ...
        case 'menuOpen':
          ...
          // 显示全屏与不全屏
          this.$emit('menuOpen', data.status)
          break
        case 'sk':
        ...
        case 'mapReset':
          view.setZoom(18)
          this.map.getView().setCenter([this.mapCenter[0], this.mapCenter[1]])
          break
        case 'zoomIn':
          view.setZoom(zoom + 1)
          break
    ...
    createMarket(data) {
      ...
      data.forEach((val, index, arr) => {
        if (val.pointRole == 'user' && this.showUserPoint && val.userId != '') {
          let icon = ''
     ...
     val.rscd != ''
          ...
          console.log('ghn=============ddddffff子系统', val)
          // 1、视频 2、图像、3、水位 4、雨量
          let icon1 = ''
          if (val.sttp == 4) {
            icon1 = require('../../../assets/images/sk/map/icon-rainfall.png')
          } else if (val.sttp == 3) {
            icon1 = require('../../../assets/images/sk/sk-blue.png')
          } else if (val.sttp == 2) {
            icon1 = require('../../../assets/images/sk/map/image.png')
          } else if (val.sttp == 1) {
            icon1 = require('../../../assets/images/sk/map/icon-video.png')
          }
          // const sw = val.sw == null ? '' : val.sw + 'm'
          // 隐患点
          feature = new Feature({
            // geometry: new OlGeomPoint([123.3, 36.3]), // 标签位置
            geometry: new OlGeomPoint([Number(val.lon), Number(val.lat)]), // 标签位置
    ...
    async addPop(data, type) {
      ...
      if (type === 'user') {
        ...
        console.log('1', userName, '2', address, '3', lng, '4', lat)
        // 点击覆盖物,弹出自定义弹框singleclick
        this.titlecloser.innerHTML = '<div >' + userName + address + '</div>'
