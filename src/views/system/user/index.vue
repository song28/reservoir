<template>
  <div
    class="app-container app-container-noHeader"
  ...
  >
      ...
      >
        ...
        <el-table-column label="序号" align="center" width="60">
          <template slot-scope="scope">
            <span>{{
              (query.start - 1) * query.pageSize + (scope.$index + 1)
            }}</span>
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column align="center" width="320" label="操作">
          <template slot-scope="scope">
            <span
              class="font-darkOrange cursor-p"
              @click.stop="dispatchRight(scope.row)"
              >分配角色</span
            >
            <span>|</span>
            <span
            ...
            <span>|</span>
            <span
              class="font-red cursor-p"
              @click.stop="handleDelete(1, scope.row)"
              >删除</span
            >
            <span>|</span>
            <span
    ...
    >
      ...
      >
        ...
        >
          <el-cascader
            v-model="adduserinfoform.devCatID"
            :options="bizTypes"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="warning" @click="resetForm('adduserinfoform')"
          >重置</el-button
        >
        <el-button @click="cancel('adduserinfoform')">取 消</el-button>
        <el-button type="primary" @click="submitForm('adduserinfoform')"
          >确 定</el-button
        >
      </div>
    </el-dialog>
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="confirmModifyRight()"
          >确 定</el-button
        >
      </span>
    </el-dialog>
...
export default {
  data() {
    ...
    var validatepassword = (rule, value, callback) => {
      ...
      } else if (value.toString().length < 4 || value.toString().length > 12) {
        callback(new Error("请输入4~12位字母或数字组合"));
      } else {
        callback();
      }
    ...
    return {
      ...
      tableLoading: true,
      tableHeight:
        window.innerHeight - (window.innerWidth / 1920) * 100 * 0.8 - 150,
  ...
  methods: {
    ...
    setAddvcd(addvcd) {
      this.selectAddvcd = this.$getUserRegion(addvcd);
    },
        ...
        this.$confirm("确定删除数据吗?", "提示", {
          ...
          doDelete(row).then(response => {
            if (response.status) {
              ...
              self.$GXprompt.notify.success(
                response.msg ? response.msg : "删除成功"
              );
            } else {
              self.$GXprompt.notify.error(
                response.msg ? response.msg : "删除失败"
              );
            }
          });
        });
