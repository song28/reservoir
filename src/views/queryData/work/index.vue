<template>
  <div class="app-container app-container-noHeader">
    <!--<div class="app-container-header">-->
      <!--站点总数：<span class="font-blue">{{totalData.total}}</span>个；-->
      <!--在线站点：<span class="font-blue">{{totalData.online}}</span>个；-->
      <!--离线站点：<span class="font-blue">{{totalData.offline}}</span>个；-->
    <!--</div>-->
    ...
    <div class="app-container-body app-container-body-column" ref="tab">
      <el-table :data="tableData" v-loading="loading" border style="width: 100%" highlight-current-row>
        ...
        <el-table-column
          ...
          label="是否在线">
          <template slot-scope="scope">
            <span v-if="scope.row.blOnline" class="font-green font-weight">在线</span>
    ...
    methods: {
      ...
      queryList() {
        ...
        let params = {
          ...
          data: this.query,
          dictIds:['009002001','0090051','009004001'],
        };
        let self = this;
        
        queryDeviceHealth(params).then(res => {
        ...
        for(let i=0; i < cdata.length; i++){
          for(let z=0; z < self.siteData.length; z++){
            if(self.siteData[z].cusno == cdata[i].cusno){
                cdata[i].blOnline = self.siteData[z].blOnline;
            }
           } 
           if(!cdata[i].blOnline || cdata[i].blOnline == "undefined"){
              cdata[i].blOnline = false;
           }
        }
        self.tableData = cdata;
          })
         }
        })
        // queryDeviceHealth
      ...
      querySiteList() {
        ...
        getSiteList(params).then(res => {
          self.siteData = res.data
          // console.info("siteData---" + res.data)
          // if (res.data.length) {
          //   self.handleRowClick(self.rowClickTable)
          // }
        })
      },
      //查询站点数据列表结束
