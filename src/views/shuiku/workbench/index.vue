<template>
  <div class="workbrench">
      ...
      <el-main>
        <div class="tb">
          ...
          >
            ...
            <el-table-column
              label="巡查（红色:超校核；橙色:超设计；黄色:超正常；蓝色:正常蓄水位以下）"
              v-if="tzType == 'xc'"
            >
              <template v-for="(item, index) in tableHeader">
                <template
                ...
                >
                  <el-table-column
                    :prop="item.prop"
                    :label="item.label"
                    :align="item.prop == 'xuncha' ? 'center' : 'left'"
                    :width="item.width"
                  ...
                  >
                    <template scope="scope">
                      ...
                      <div @click="clickSw(scope.row, 'patrol', item.prop)">
                        <div
                          v-if="
                            item.prop == 'sw' &&
                        ...
                        >
                          <el-image
                            ...
                            :src="
                              require('@/assets/images/sk/alarmLamp-red.gif')
                            "
                          ></el-image>
                          {{ scope.row.sw }}
                        </div>
                        ...
                        >
                          <el-image
                            style="width:30px; height:30px"
                            :src="
                              require('@/assets/images/sk/alarmLamp-orange.gif')
                            "
                          ></el-image>
                          {{ scope.row.sw }}
                        </div>
                        ...
                        >
                          <el-image
                            style="width:30px; height:30px"
                            :src="
                              require('@/assets/images/sk/alarmLamp-yellow.gif')
                            "
                          >
                          </el-image>
                          {{ scope.row.sw }}
                        ...
                        <div
                          v-if="item.prop == 'sw' && scope.row.sw == ''"
                        ></div>
                      </div>
                      <!--                      <p v-if="item.prop == 'sw'">-->
                      <!--                        {{ scope.row.sw }}-->
                      <!--                      </p>-->
                      <!-- 坝高-变幅 -->
                      <p v-if="item.prop == 'bf'" style="">
                        <span v-if="scope.row.bf > 0" style="color:red">
                          ↑&nbsp;{{ scope.row.bf }}
                        </span>
                        <span v-if="scope.row.bf < 0">
                          ↓&nbsp;{{
                            scope.row.bf.substring(1, scope.row.bf.length)
                          }}
                        </span>
                        <span v-if="scope.row.bf == 0">
                          {{ scope.row.bf }}
    ...
    <!-- <map-dialog
      ...
      @closeDialogMap="closeDialogMap">    </map-dialog> -->
    <el-dialog title="提示" :visible.sync="dialogDataSHMB" width="60%">
      <DetailSkList :data="skdata"></DetailSkList>
    </el-dialog>
  </div>
</template>
<script>
...
import DetailSkList from "../reservoirManagement/common/detailSkList";
...
export default {
  ...
  methods: {
    ...
    clickSw(row, type, filed) {
      this.dialogDataSHMB = true;
      this.skdata = new Object();
      this.skdata.data = {
        gid: row.gid,
        code: row.gid,
        name: row.name
      };
      this.skdata.type = "detail";

      console.log(row, type, filed, "水位值");
    },
    closeDialogMap() {
      this.dialogDataSHMB = false;
    },
    //巡查跳转巡查记录
