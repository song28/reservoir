<template>
  <div class="skListForm" v-if="isShow">
    <div class="tips" v-if="data.type == 'add'">
      温馨提示：当前处于添加状态！！！
    </div>
    <el-form
      :model="formData"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        addvcd: [
          { required: false, message: "请选择行政区", trigger: "change" },
        ],
      },
    };
  },
      ...
      try {
        if (status) {
          if (obj == null) {
            ...
            if (obj.enmnit == "05") {
              let channel = obj.devicesno.substr(
                obj.devicesno.lastIndexOf("_") + 1
              );
              if (obj.monittyp == "3") {
            ...
            this.getRsrEnmnitList();
            this.formData = obj;
            this.getTableHeader(obj.enmnit);
            console.log(obj, "666666666666666666666666666");
          }
  ...
  watch: {
    ...
    data: {
      handler(val) {
        console.log(val, "val----------why????????????");
        if (val.nodeTreeData.enmnit == "99") {
          this.noPam = true;
          this.isShow = true;
          this.disabled = false;
        } else {
          console.log(val, "val-----------------9999999999999999");
          this.noPam = false;
