<template>
  <div class="skListForm">
    <div class="skListForm-left">
      <el-form :model="formData" ref="formData" label-width="200px" :rules="rules">
        <el-row :gutter="20">
          <template v-for="(item, index) in benefitHeader">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        ctcd: [
          { required: false, message: "请输入所在流域代码", trigger: "blur" },
          { max: 2, message: "最大2个字符", trigger: "blur" },
        ],
        rvcd: [
          { required: false, message: "请输入所在河流代码", trigger: "blur" },
        ...
        // bsssin: [
        //   { required: false, message: "请输入地震基本烈度", trigger: "change" },
        ...
        dtpl: [
          { required: false, message: "请输入水准基面", trigger: "blur" },
          { max: 2, message: "最大2个字符", trigger: "blur" },
        ],
  ...
  methods: {
    ...
    handleSave() {
      this.$refs["formData"].validate((valid) => {
        if (valid) {
          ...
          if (this.type == "add") {
            this.saveWrpRsrBnFun(params);
          } else {
            this.updateWrpRsrBnFun(params);
          }
        } else {
    ...
    // 新增
    saveWrpRsrBnFun(params) {
      saveWrpRsrBn(params).then((res) => {
        if (res.status) {
          this.$notify.success({
            title: "成功",
            message: res.msg,
    ...
    // 编辑
    updateWrpRsrBnFun(params) {
      updateWrpRsrBn(params).then((res) => {
        if (res.status) {
          this.$notify.success({
            title: "成功",
            message: res.msg,
    ...
    // 查询
    async loadWrpRsrBnDetail() {
      let params = {
        rscd: this.data.data.rscd ? this.data.data.rscd : this.rscd,
      };
      let { status, obj, msg } = await loadWrpRsrBnDetail(params);
      if (status == 200) {
        if (!obj) {
          let rscd = this.data.data.rscd ? this.data.data.rscd : this.rscd;
          this.$set(this.formData, "rscd", rscd);
          this.type = "add";
