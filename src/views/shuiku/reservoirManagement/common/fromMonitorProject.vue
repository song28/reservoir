      ...
      <el-row :gutter="20">
        <template v-for="(item, index) in monitorProjectHeader">
          ...
          <el-col
            ...
            v-if="
              item.prop != 'posname' &&
              item.prop != 'enmnitName' &&
              item.prop != 'monittypName'
            "
          >
...
import { getBaseInfoById, saveRsrEnmint, getRsrPosListByRscd } from "../api";
...
export default {
  ...
  methods: {
    ...
    async getRsrPosListByRscd() {
      ...
      let { status, data, msg } = await getRsrPosListByRscd(params);
      if (status) {
        data.forEach((item) => {
          engineeringPartsOptions.forEach((ite) => {
            if (item.poscode == ite.value) {
              item.label = ite.label;
              item.value = ite.value;
            }
          });
        });
        this.epOptions = data ? data : [];
      }
    },
  },
  watch: {
