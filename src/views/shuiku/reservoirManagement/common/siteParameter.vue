  ...
  <div class="skListForm">
    <el-form :model="formData" ref="formData" label-width="170px" :rules="rules">
      <el-row :gutter="20" v-if="!noPam">
        <template v-for="(item, index) in parameterHeader">
          <el-col :span="12" :key="index">
            <el-form-item
              v-if="item.prop == 'instrtype'"
              label="仪器类型"
              prop="apparatustype"
            >
              <el-select
                v-model="formData.instrtype"
                size="mini"
                filterable
                placeholder="请选择"
              >
                <el-option
                  v-for="item in apparatustypeOptions"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                >
            ...
            <!-- 监测方式 -->
            <el-form-item
              v-else-if="item.prop == 'monitormode'"
              :label="item.label"
              :prop="item.prop"
            >
              <!-- :disabled="disabled" -->
              ...
              >
                <el-option
                  v-for="item in monitoringWayOptions"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                >
            ...
            <!-- 采集单元 -->
            <el-form-item
              v-else-if="item.prop == 'collectunit'"
              :label="item.label"
              :prop="item.prop"
            >
              <el-select
...
export default {
  ...
  methods: {
    ...
    async savePointDeviceParam() {
      ...
      let { status, data, msg } = await savePointDeviceParam(params);
      if (status) {
        this.$notify.success({
          title: "成功",
          message: msg,
    ...
    async saveRsrSection() {
      ...
      console.log(params, "params", enmnitid, "enmnitid");
    ...
    handleChangeEnmnit(val) {
      console.log(val, "Enmnit6666");
      this.enmnit = val;
      this.getTableHeader(val);
    },
    async getRsrEnmnitList() {
      console.log(this.data.nodeTreeData, "this.data.nodeTreeData");
      let params = {
    ...
    // 查询测点的详细信息
    async getPointDeviceParam() {
      let params = {
        enmnitcode: this.data.data.enmnit,
        pointid: this.data.data.id,
      };
      let { status, obj, msg } = await getPointDeviceParam(params);
      if (status) {
        if (!obj) {
          this.isShow = true;
        } else {
