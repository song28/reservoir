<template>
  <div class="skListForm">
    <div class="skListForm-left">
      <el-form :model="formData" ref="formData" label-width="135px" :rules="rules">
        <el-row :gutter="20">
          <template v-for="(item, index) in spillWayHeader">
...
import { getBaseInfoById, saveSw, loadSwDetail, updateSw } from "../api";
...
export default {
  ...
  methods: {
    ...
    handleSave() {
      this.$refs["formData"].validate((valid) => {
        if (valid) {
          ...
          if (this.type == "add") {
            this.saveSwFun(params);
          } else {
            this.updateSwFun(params);
          }
        } else {
    ...
    async loadSwDetail() {
      ...
      let { status, obj, msg } = await loadSwDetail(params);
      if (status == 200) {
        if (!obj) {
