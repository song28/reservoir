<template>
  <div class="skListForm">
      ...
      <el-form ref="formData" :model="formData" label-width="140px" :rules="rules">
        <el-row :gutter="20">
          <template v-for="(item, index) in basicDataHeader">
            <el-col v-if="item.prop == 'rsov'" :key="index" :span="8">
            ...
            <el-col v-else-if="item.prop == 'focus'" :key="index" :span="8">
              <el-form-item
                v-if="item.prop == 'focus'"
                label="是否重点"
                prop="focus"
              >
                <el-select
            ...
            <el-col v-else-if="item.prop == 'sickness'" :key="index" :span="8">
              <el-form-item
                v-if="item.prop == 'sickness'"
                label="是否病险"
                prop="focus"
              >
                <el-select
                  v-model="formData.sickness"
                  :disabled="disabled"
                  size="mini"
                  filterable
                  placeholder="请选择"
    ...
    <div v-if="this.data.type != 'detail'" class="skListForm-btns">
      <el-upload
        class="upload-demo"
        :action="serveUrl"
        :headers="header"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        ctcd: [
          { required: true, message: "请选择所在流域", trigger: "change" },
          { max: 2, message: "最大2个字符", trigger: "change" },
        ],
        rvcd: [
          // { required: true, message: "请输入所在河流", trigger: "blur" },
  ...
  methods: {
    ...
    async loadRsvrInfo() {
      ...
      const { status, obj, msg } = await loadRsvrInfo(params);
      if (status) {
