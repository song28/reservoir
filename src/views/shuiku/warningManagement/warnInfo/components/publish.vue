<template>
  <div class="warn" style="width: 100%">
    <!--  -->
    <span v-if="showBack" style="color: #29958b; font-weight: bold" @click="back"
      >＞＞返回</span
    >
    <div>
      <el-button
        ...
        plain
        size="mini"
        style="position: absolute; right: 4.8rem; top: 0.7rem; z-index: 999"
        @click="addSms()"
        >存为模板信息</el-button
      >
      <el-button
        ...
        plain
        size="mini"
        style="position: absolute; right: 3.2rem; top: 0.7rem; z-index: 999"
        @click.stop="handleSms()"
        >选择模板</el-button
      >
    </div>
    <el-form ref="form" :model="form" size="small" style>
      <el-form-item label="预警发布等级：" class>
        <el-radio-group v-model="form.warnLevel">
          <!-- <el-radio id="blueWarn" :label="1">蓝色预警</el-radio>
          <el-radio id="yellowWarn" :label="2">黄色预警</el-radio> -->
          <el-radio id="orangeWarn" :label="3">橙色预警</el-radio>
          <el-radio id="redWarn" :label="4">红色预警</el-radio>
        </el-radio-group>
      </el-form-item>
      ...
      <el-form-item>
        <el-button type="primary" icon="el-icon-plus" size="mini" @click="addDiy()"
          >添加自定义对象</el-button
        >
        <el-button type="primary" icon="el-icon-plus" size="mini" @click="addtem()"
          >添加临时接收对象</el-button
        >
      </el-form-item>
      <el-form-item>
          ...
          <el-tabs type="card" style="width: 100%">
            <el-tab-pane v-if="msgwarn" label="接收人">
              ...
              >
                ...
                <el-table-column label="职责" align="center" prop="dutyPerson" />
    ...
    >
      <div v-if="diyDilog" class="table-list diyDialog">
        <el-form
          ...
          :model="formDiy"
          label-width="100px"
          class="demo-form-inline mt-18"
        >
          <el-row :gutter="20">
            <el-col :span="12">
              <el-form-item label="姓　　名：">
                <el-input v-model="formDiy.name" class="0" size="mini" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="电　　话：">
                <el-input v-model="formDiy.mobile" class size="mini" />
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="20">
            ...
            <el-col :span="12">
              <el-form-item label="职　　务：">
                <el-input v-model="formDiy.duty" class size="mini" autocomplete="off" />
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="20">
            <el-col :span="24">
              <el-form-item label="单位名称：">
                <el-input v-model="formDiy.unit" class size="mini" autocomplete="off" />
              </el-form-item>
            </el-col>
          </el-row>
        </el-form>
    ...
    <el-dialog
      ...
      :close-on-click-modal="false"
    >
      <div v-if="smsDialog" class="table-list mt-18 pl-20 diyDialog">
        <el-table
          :data="smsList"
          style="width: calc(100% - 3px); border: 1px solid rgba(234, 231, 237, 1)"
          height="400px"
        ...
        >
          ...
          <el-table-column label="" align="center" width="55">
            <template slot-scope="scope">
              <span>
                <el-radio v-model="form.warnPublishContent" :label="scope.row.content"
                  >&nbsp; 　
                </el-radio>
              </span>
            </template>
          ...
          >
            <template slot-scope="scope">
              <el-button
                ...
                size="mini"
                v-if="!scope.row.edit"
                @click="update(scope.row, 'edit', scope.$index)"
                >修改</el-button
              >
              <el-button
                type="primary"
                size="mini"
                v-if="scope.row.edit"
                @click="update(scope.row, 'save', scope.$index)"
                >保存</el-button
              >
              <el-button type="danger" size="mini" @click="delSms(scope.row)"
                >删除</el-button
              >
            </template>
          </el-table-column>
        </el-table>
        <!-- <el-radio-group v-model="form.warnPublishContent">
          <el-radio :label="3" v-for="(item, index) in smsList" :key="index">{{
            item.content
          }}</el-radio>
      ...
      <span slot="footer" class="dialog-footer">
        <!-- <el-button size="mini" @click="temporaryDialog = false">取 消</el-button> -->
        <el-button type="primary" size="mini" @click="smsDialog = false">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
<script>
import { loadPersonlist4Page } from "@/views/shuiku/reservoirManagement/api";
import {
  customSend,
  addSmsTemplate,
  updateSmsTemplate,
...
export default {
  ...
  methods: {
    handleSms() {
      this.smsDialog = true;
      let params = {};
      selectSmsTemplate(params).then((res) => {
        if (res.status) {
          res.data.forEach((d) => {
            d.edit = false;
          });
          this.smsList = res.data ? res.data : [];
        } else {
        }
      });
    },
    update(row, type, index) {
      console.log(row, type, index, "row, type, index");
      if (type == "save") {
        let params = row;
        updateSmsTemplate(params).then((res) => {
          if (res.status) {
            this.$GXprompt.notify.success("修改成功");
            this.smsList[index].edit = false;
          } else {
            this.$GXprompt.notify.error("修改失败");
          }
        });
      } else {
        this.smsList[index].edit = true;
      }
    },
    delSms(row) {
      let params = {
    ...
    async getPersonlist() {
      ...
      try {
        ...
        if (status) {
          ...
          this.tableDataOfPerson = res.data ? res.data : [];
    ...
    postSaveHandle() {
      ...
      let params = {
        id: "",
        warnType: "",
        warnReason: this.form.warnPublishContent,
        createtime: "",
        warnStatus: "",
        warnLevel: this.form.warnLevel,
        rscd: this.data.rscd,
        mpcd: "",
        deviceId: "",
        cusno: "",
        ...
        //   },
        // ],
        sendContext: this.form.warnPublishContent,
        sendType: "",
        sendUser: "",
        sendUserName: "",
      };
      if (this.form.smsRecords.length) {
        console.log(this.form.smsRecords, "this.form.smsRecords");
        this.form.smsRecords.forEach((d) => {
          params.warnSendUsers.push({ name: d.name, phone: d.mobile });
        });
      } else {
        this.$GXprompt.notify.error("请选择短信接收人或设备");
        return
      ...
      if (!this.data.rscd) {
        this.$GXprompt.notify.error("请选择水库");
      }
      customSend(params).then((res) => {
        if (res.status) {
          this.$GXprompt.notify.success("操作成功！");
        } else {
          this.$GXprompt.notify.error("操作失败！");
        }
      });
    },
    handelWarn(data) {
    ...
    // 隐患点下的预警接收人查询接口
    async queryReceiver(hdpCode, level) {
      let pam = {
        name: "",
        rscd: this.data.rscd,
        page: 1,
      ...
      try {
        ...
        if (status) {
          ...
          this.tableDataOfPerson = data ? data : [];
          console.log(this.tableDataOfPerson, "this.tableDataOfPerson");
          this.tableLoading = false;
        } else {
          this.$notify.error({
            title: "失败",
    ...
    handleSelectionPerson(val) {
      console.log(val, "xuanzhong");
      this.form.smsRecords = [];
      val.forEach((d) => {
        this.form.smsRecords.push(d);
      });
    ...
    addRow() {
      ...
      var list = this.formDiy;
      let isAdd = true;
      if (this.tableDataOfPerson.length > 0) {
        this.tableDataOfPerson.forEach((d) => {
          if (d.name == this.formDiy.name && d.mobile == this.formDiy.mobile) {
    ...
    addSms() {
      if (!this.form.warnPublishContent) {
        this.$GXprompt.notify.warning("请先填写预警发布内容");
        return;
      }
      let params = {
        templatename: "",
        content: this.form.warnPublishContent,
      };
...
.table-list {
  ...
  .el-tabs--card > .el-tabs__header .el-tabs__nav .el-tabs__item.is-active::before {
    content: "";
    // height: 2px;
    height: 0 !important;
    width: 100%;
  ...
  .el-tabs--card > .el-tabs__header .el-tabs__nav .el-tabs__item.is-active::after {
    content: "";
    width: 0;
    height: 0;
    // border: 4px solid;
