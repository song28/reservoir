...
import warnMain from "./components/warnMain";
...
export default {
  components: { damLeftTreeBar, Btns, Dialog, skDialog, warnMain },
  data() {
    return {
      pickerOptions: {
        disabledDate: (time) => {
  ...
  methods: {
    ...
    async findEarlyWarnConfByPage() {
      ...
      let { status, data, msg } = await findEarlyWarnConfByPage(params);
      if (status) {
        self.tableData = data;
      }
    },
