 ...
 * @LastEditTime: 2021-09-07 10:19:19
-->
<template>
  ...
  watch: {
    'chartData.data': {
      handler: function(newV, oldV) {
        ...
        const lineLegend = []
        Object.keys(this.chartData.data.y).forEach((e, i, a) => {
          lineLegend.push('UP' + (i + 1))
        })
        const info = {
          icon: 'dc-dblf',
          Ykind: 'single', // Y轴单坐标
          idname: this.chartId,
          type: ['line', 'line'],
          lineColor: ['#14AFCB', '#FF10E7'],
          lineLegend: lineLegend, // 图例
          Yname: ['渗流压力（m）'], // Y轴纵坐标单位
          Xname: ['时间'],
