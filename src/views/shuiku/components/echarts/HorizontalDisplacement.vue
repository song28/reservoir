 ...
 * @LastEditTime: 2021-09-14 15:34:37
-->
<template>
  ...
  watch: {
    'chartData.data': {
      handler: function(newV, oldV) {
        console.log('gdagggdgds=============yyyy')
        console.log(this.chartData, 'chart=============yyyy', this.chartData.data.y)
        ...
        const info = {
          ...
          idname: this.chartId,
          type: ['line', 'line', 'line'],
          lineColor: ['#14AFCB', '#FF10E7', '#C4433E', '#2F4455', '#60A1A7'],
  ...
  methods: {
    draw(infoConfig) {
      ...
      if (infoConfig.Ykind == 'double') {
        ...
        option.series = []
        if (Object.values(this_.chartData.data.y).length > 0) {
          console.log(infoConfig['dataY0'], '0000')
          Object.values(this_.chartData.data.y).forEach((val, index, arr) => {
            const x = infoConfig['dataY' + index].x
            // const y = infoConfig['dataY' + index].y
            // const z = infoConfig['dataY' + index].z
            console.log(x, '111')
            // console.log(y, '333')
            // console.log(z, '444')
