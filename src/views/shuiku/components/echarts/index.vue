<template>
  <div class="chart">
    <div v-if="chartData.type" class="rainC">
      <div class="rainChart">
        <div
          :id="chartId"
          ref="chart"
...
export default {
    ...
    'chartData.data': {
      handler: function(newV, oldV) {
        ...
        const info = {
          ...
          Yname: ['库容(万m³)'], // Y轴纵坐标单位
          Xname: ['水位(m)'],
          specialData: this.chartData.specialData,
  ...
  methods: {
    draw(infoConfig) {
      ...
      const myChart = this.$echarts.init(document.getElementById(infoConfig.idname))
      const this_ = this
      let option = {}
          ...
          infoConfig.type.forEach((val, index, arr) => {
            option.series.push({
              ...
              markPoint:
              {
                ...
                // 设置符号大小
                symbolSize: [140, 100], // 容器大小
                data: [
                  {name: '正常蓄水位', value: '正常蓄水位\n' + infoConfig.specialData.zcxsw + '(m)', xAxis: infoConfig.specialData.zcxsw, yAxis: infoConfig.specialData.zcxswV, itemStyle: {
                    color: '#0e72dd'
                  }},
                  {name: '设计洪水位', value: '设计洪水位\n' + infoConfig.specialData.sjhsw + '(m)', xAxis: infoConfig.specialData.sjhsw, yAxis: infoConfig.specialData.sjhswV, itemStyle: {
                    color: 'orange'
                  }},
                  {name: '校核洪水位', value: '校核洪水位\n' + infoConfig.specialData.jhhsw + '(m)', xAxis: infoConfig.specialData.jhhsw, yAxis: infoConfig.specialData.jhhswV, itemStyle: {
                    color: 'red'
                  }},
                  {name: '汛限水位', value: '汛限水位\n' + infoConfig.specialData.xxsw + '(m)', xAxis: infoConfig.specialData.xxsw, yAxis: infoConfig.specialData.xxswV, itemStyle: {
                    color: '#e6cb2e'
                  }}
                ]
              }
      ...
      option.tooltip = {
        ...
        formatter: function(params) {
          return '水位:' + params[0].axisValue + '（m）</br>' + params[0].seriesName + ':' + params[0].value + '(万m³)'
        }
      }
      option.grid = {
        show: true,
        ...
        feature: {
          ...
          dataView: {
            ...
            optionToContent: function(opt) {
              ...
              var tdHeads = '<td  style="padding:0 10px">' + infoConfig.Xname + '</td>'
              series.forEach(function(item) {
                tdHeads += '<td style="padding: 0 10px">' + infoConfig.Yname + '</td>'
              })
              var table =
