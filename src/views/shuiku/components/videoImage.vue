 ...
 * @LastEditTime: 2021-10-19 16:28:37
-->
<template>
    ...
    <div class="left">
      ...
      <div class="leftCon">
        <div v-if="activeName=='one'" class="videoItem">
          ...
          <template v-else>
            <div class="item">
              <template v-if="curvideo.extranetPath">
                <iframe :src="curvideo.extranetPath" width="100%" height="100%" />
              </template>
              <template v-else>
                <video
                  id="videoElement"
      ...
      <div class="first">
        ...
        <div v-if="activeName == 'one'" class="contralBtn">
          ...
          <div class="con">
            ...
            >
              ...
              <el-col :span="8">
                <template v-if="videoBtnList[index][1].type=='reset'">
                  <el-popover
                    placement="bottom"
                    title="预设点"
                    trigger="click"
                    ...
                    @show="showReset"
                  >
                    <template v-if="presetPoint==''|| presetPoint==null">
                      暂未设置预设点
                    </template>
                    <template v-else>
                      <el-radio-group v-model="radio" style="width:150px;white-space:normal !important;">
                        <template v-for="(item,index) in presetPoint">
                          <el-radio :label="item.PresetID">  {{ item.PresetName }} </el-radio>
                        </template>
                      </el-radio-group>
                    </template>
        ...
        <div :class="[activeName == 'one' ? 'contralSite' : 'contralSiteImage']">
          ...
          <div class="con">
            ...
            >
              <el-col :span="24">
                <p :class="[current==index?'chooseSiteVideo':'SiteVideo']" @click="clickSiteVideo(item,index)">{{ item.name }}</p>
              </el-col>
            </el-row>
            <el-row
  ...
  watch: {
    'radio': {
      handler: async function(newV, oldV) {
        ...
        // 设置（set）、移动到预置点（goto）、删除（remove）预置位编号(1~255)
        const pam = {
        ...
        const {data, status} = await preset(pam)
        if (status) {
        ...
        if (newV.length) {
          ...
          if (this.activeName == 'one') {
            if (videoList.length > 0) {
              this.videoId = videoList[0].id
              this.curvideo = videoList[0]
              this.$nextTick(() => {
                this.playVideo()
              })
            }
  ...
  methods: {
    ...
    playVideo() {
      if (flvjs.isSupported()) {
        if (!this.curvideo.extranetPath) {
          const str =
      'rtsp://' +
      this.curvideo.deviceUsername +
      ':' +
      this.curvideo.devicePassword +
      '@' +
      this.curvideo.deviceIp +
      '/' +
      this.curvideo.devicePort
          const url =
      this.curvideo.servicePath + '/' + window.btoa(str) + '/live.flv'
          this.startPlay('videoElement', url)
        }
      }
    },
    startPlay(id, url) {
    ...
    // 预设位置控制
    async showReset() {
      this.presetPoint = null
      // 预设点
      const pam = {
        videoId: this.videoId
      }
      const {data: resetvideoDataGq, status: statusReset} = await fetchpreset(
        pam
      )
      if (statusReset) {
        if (resetvideoDataGq != '' || resetvideoDataGq != null) {
          this.presetPoint = JSON.parse(resetvideoDataGq).PresetItemList
      ...
      if (typeof data === 'number') {
        ...
        if (status) {
          ...
          if (mouseType == 'click' || mouseType == 'up') {
            ...
            const {data: stopvideoData, status: statusData} = await ptzStop(
              stopPam
            )
            if (statusData) {
