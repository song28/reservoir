<template>
  <div class="app-container app-container-noHeader">
    <div class="app-container-search">
      <filter-toggler @query="seachByName" @reset="resetList" :hasMore="false">
    ...
    <div class="app-container-body" ref="tab">
      ...
      <el-table :data="tableData" v-loading="tableLoading" border style="width: 100%" :height="tableHeight">
        <el-table-column
          label="序号"
          align="center"
          width="60">
    ...
    <el-dialog title="添加角色" :visible.sync="dialogFormVisible" :before-close="handleClose" :close-on-click-modal="false">
    ...
    <el-dialog title="修改角色" :visible.sync="dialogFormVisible_edit" :before-close="handleClose_edit" :close-on-click-modal="false">
    ...
    <el-dialog :visible.sync="showDispatchRight" :close-on-click-modal="false"
...
<script>
  import {getList, seachByName, submitForm, doDelete, getRightTreeList, getRolePermissions, saveRight} from './api'
    ...
    methods: {
      dispatchRight(row) {
        ...
        getRolePermissions(row.id).then(function (response) {
          if (response.status == true) {
            ...
            self.$nextTick(() => {
              self.$refs.rightTree.setCheckedKeys(self.currentRolePermissions);
            })
      ...
      queryList(callBack) {
        ...
        getList().then(function (res) {
          ...
          if (res.status) { //0表示成功，1表示失败
            callBack(res.data, res.data.length);
          } else {
